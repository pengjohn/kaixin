...
void* KaiXinAPI_RanchList_JsonParse(char *text)
{
	cJSON *json;
	cJSON *pTemp0;
	tResponseRanchList*  Response = new tResponseRanchList;
	memset(Response, 0 , sizeof(tResponseRanchList));
	...
	if(Response->ret == 1)
	{
		...
		if(pTemp0)
		{
			...
			if( nSize1 != 0 )
			{
				...
				Response->friends = (RanchList_friends*) malloc(sizeof( RanchList_friends ) * nSize1 );
				memset(Response->friends, 0 , sizeof(RanchList_friends) * nSize1 );
			}
			for ( i = 0; i < nSize1; i++ )
			{
...
// 构造函数
TRanchListForm::TRanchListForm(TApplication* pApp):TWindow(pApp)
{
	Create(APP_KA_ID_RanchListForm);
}

// 析构函数
TRanchListForm::~TRanchListForm(void)
{
	if( Response )
	{
		delete Response;	
...
// 窗口事件处理
Boolean TRanchListForm::EventHandler(TApplication * pApp, EventType * pEvent)
{
...
// 窗口初始化
Boolean TRanchListForm::_OnWinInitEvent(TApplication * pApp, EventType * pEvent)
{
	int iRet = eFailed;
	int nIndex = 0;
	...
	{
		TBarRow *lpRow = NULL;
		TCoolBarList* pCoolBarList = static_cast<TCoolBarList*>(GetControlPtr(APP_KA_ID_RanchListForm_RanchListCoolBarList));
		if (pCoolBarList)
		{
			...
			if (lpRowList)
			{
				...
				if(lpRow)
				{
					...
					if(lpItem)	
					{
						...
						TUString::StrPrintF(pszSubTitle, TResource::LoadConstString(APP_KA_ID_STRING_SubTitleRanch),TResource::LoadConstString(APP_KA_ID_STRING_Friend));
					...
					if(nListItems == 0)
					{
						...
						if(lpItem)
						{
							...
							pNoneHarvest->SetCaption(TResource::LoadConstString(APP_KA_ID_STRING_NoneHarvestRanch),FALSE);
...
// 关闭窗口时，保存设置信息
Boolean TRanchListForm::_OnWinClose(TApplication * pApp, EventType * pEvent)
{
	return TRUE;
}
...
// 控件点击事件处理
Boolean TRanchListForm::_OnCtrlSelectEvent(TApplication * pApp, EventType * pEvent)
{
	Boolean bHandled;
	...
	for( int nIndex = 0; nIndex < nListItems; nIndex++)
	{
		if(nFriendNameId[nIndex] == nCtrlID)
		{
			if(strcmp(Response->friends[nIndex].mature, "1") == 0)//可收获
			{
				...
				Set_Url_Params(KX_RanchDetail, "fuid", (char*)sFUid);				
